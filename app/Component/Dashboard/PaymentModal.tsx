...
const PaymentModal = ({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      const renamedFiles = _files.map((file, index) => {
        ...
        const newFileName = `buktiPembayaran_${id}_${originalFileName}`;
  ...
  const handleSubmit = async(e: FormEvent<HTMLFormElement>) => {
    ...
    await axios.post("/api/upload", formData);
        await axios.post('/api/buktiPembayaran', {
        id: id,
      ...
      <div className="min-h-[250px] min-w-[500px] bg-white shadow-2xl rounded-xl p-5 flex flex-col items-center justify-around gap-5">
        <div className="flex flex-col justify-center items-center gap-2">
          ...
          <p className="font-light text-sm px-4">
            Transfer Ke Rekening : 0128290812 atas nama : askjsanaskjnskasnkjan
            dan masukkan bukti pembayaran
          </p>
        ...
        <div className="flex gap-5">
          <button
            className="border-2 border-[#1988db] py-2 px-10 rounded-xl font-semibold text-lg text-[#1988db] hover-bg-[#1988db] hover:bg-[#1988db] hover:text-white transition-colors duration-75"
